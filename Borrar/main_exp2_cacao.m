...
Nseg = 512; %512; % Number of Superpixels: one value or vector
% for pw=0:12
%     Nseg = [Nseg, pow2(8,pw)];
%     try
%         hadamard(Nseg(pw+1));
...
switch(database)
    case 1          % Load Cacao Mix1
        load('mix1_mod.mat')
        load('mix1_mod_gt.mat')
        filename = 'Cacao_Mix1';
        data_lbls = mix1_gt(:);% Groundtruth
        clear mix1_gt;
    case 2          % Load Cacao Mix2
        load('mix2_mod.mat')
        load('mix2_mod_gt.mat')
        filename = 'Cacao_Mix2';
        data_lbls = mix2_gt(:);% Groundtruth
        clear mix2_gt;
end
...
for nExp = 1:1 %number of realizations
    for nseg = Nseg % variando numero de superpixels
        for noise = Noise
            ...
            %Add noise
            if noise>0
                g = awgn(g,noise,'measured');
                g_random = awgn(g_random,noise,'measured');
            end
            ...
            %% Single Pixel Proposed Fast Reconstruction
            rstl_name = "Prop_Fast";
            save_flname = ['Results/Noise/',filename,'_nseg='...
